...
async function initPlayer() {
  if (shakaUi && shakaUi.destroy) {
    try { await shakaUi.destroy(); } catch (e) { debugLine('Shaka UI destroy failed', e); }
    shakaUi = null;
  }
  if (player) {
  ...
  // Native Shaka controls: no custom overlay/focus engine. This is the only TV UI.
  try {
    if (window.shaka && shaka.ui && shaka.ui.Overlay && videoContainer) {
      shakaUi = new shaka.ui.Overlay(player, videoContainer, video);
      shakaUi.configure({
        addSeekBar: true,
        ...
        seekBarColors: {
          ...
          buffered: 'rgba(255,255,255,0.55)',
      ...
      debugLine('Native Shaka UI overlay initialized');
    } else {
      debugLine('Shaka UI unavailable; playback will continue without native UI controls');
    }
  } catch (e) {
    debugLine('Native Shaka UI init failed', e);
  }
...
async function loadContent(mediaInfo) {
  ...
  if (customData.debugOnError) autoDebugOnError = true;
